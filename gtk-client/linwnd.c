...
GtkWidget *create_login_wnd()
{
	...
	gtk_window_set_title(GTK_WINDOW(login_window), "IM即时通讯系统");
	...
	gtk_entry_set_text(GTK_ENTRY (entry_user_id), "0610031007");
	...
	gtk_entry_set_text(GTK_ENTRY (entry_pass), "123456");
...
GtkWidget *create_linpopwnd()
{
	...
	gtk_window_set_title(GTK_WINDOW(linpop_window), "IM即时通讯系统");
	g_signal_connect (G_OBJECT (linpop_window), "destroy",
			G_CALLBACK (linpop_quit), NULL);
	gtk_widget_set_size_request(linpop_window, 285, 625);
